...
import (
	...
	"github.com/Michael-Wilburn/greenlight/internal/validator"
	"github.com/lib/pq"
)
...
func (m MovieModel) Get(id int64) (*Movie, error) {
	/* The PostgreSQL bigserial type that we're using fot the movie ID starts\
	   auto-incremeting at 1 by default, so we know that no movies will have ID values
	   less than that. To avoid making an unnecessary database call, we take a shortcut
	   and return and ErrRecordNotFound error straght away.
	*/
	if id < 1 {
	...
	/* Use the context.WithTimeOut() function to create a context.Context which carries a
	   3-seconds timeout deadline. Note that we're using the empty context.Background()
	   as the 'parent' context
	*/
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)

	// Importantly, use defer to make sure that we cancel the context befire the Get() method returns.
	defer cancel()
...
func (m MovieModel) Update(movie *Movie) error {
	// Declare the SQL query for updating the record and returning the version number
...
func (m MovieModel) Delete(id int64) error {
	...
	/* Execute the SQL query using the Exec() method, passing in the id variable as
	   the value for the placeholder parameter. The Exec() method returns
	   a sql.Result object.
	...
	*/
	...
	/* If no rows were affected, we know that the movies table didn't contain a record
	   with the provided ID at the moment we tried to deleteit. In that case we
...
func (m MovieModel) GetAll(title string, genres []string, filters Filters) ([]*Movie, error) {
	...
	FROM movies WHERE (to_tsvector('simple', title) @@ plainto_tsquery('simple', $1) OR $1 = '') AND (genres @> $2 OR $2 = '{}') ORDER BY id`
